...
import { PrivateChannelInvites } from "@/lib/db/schema";
...
function ChannelInviteModal({
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      const invite = await fetcher<PrivateChannelInvites>(
      ...
      const url = `${window.location.origin}/auth/channel-invite/${invite.code}`;
      setInviteLink(url);
    } catch (error) {
      console.error(error);
    } finally {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Share this link with workspace members to grant them access to this
            private channel.
          </DialogDescription>
        </DialogHeader>
          ...
          {!inviteLink ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              <p className="text-xs text-muted-foreground">
                Anyone with this link will be able to join #{channelName}. It
                expires in 7 days.
              </p>
            </div>
          )}
...
export function ChannelInviteModalWrapper(props: ChannelInviteModalProps) {
  return (
